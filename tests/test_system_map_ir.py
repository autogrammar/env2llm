...
from env2llm.system_map_bridge import doql_file_to_system_map, task_context_to_system_map
from env2llm.system_map_generator import generate_system_map
from env2llm.system_map_ir import (
    CommandSchemaIR,
    FieldSpec,
    MimeTypeSpec,
    RuntimeSpecIR,
...
from env2llm.system_map_runtimes import build_runtimes_for_example, resolve_command_runtime
...
def test_task_context_to_system_map_and_render(invoice_example_dir) -> None:
    ctx = collect_task_context(invoice_example_dir, example_name="01-invoice", environment={})
    ir = task_context_to_system_map(ctx, example_dir=invoice_example_dir)
...
def test_build_runtimes_for_01_invoice(invoice_example_dir) -> None:
    runtimes = build_runtimes_for_example(
        "01-invoice",
        example_dir=invoice_example_dir,
...
def test_generate_system_map_01_invoice(invoice_example_dir) -> None:
    ir = generate_system_map(invoice_example_dir, example_id="01-invoice")
    assert any(c.name == "send_invoice" for c in ir.commands)
    assert ir.runtimes
